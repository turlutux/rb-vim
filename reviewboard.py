...
def get_review_ship_it(server):
    ...
    @param server an instance of RB server
    """
    if options.rid:
        url = 'api/review-requests/%s/reviews/' % (options.rid)
...
def list_review_comment(server):
    ...
    for i in rsp['reviews']:
        ...
        comment_rsp = server.api_get(i['links']['diff_comments']['href'])
        ...
        for j in comment_rsp['diff_comments']:
            ...
            #print "U"*10
            try:
                filename_rsp = server.api_get(j['links']['filediff']['href'])
